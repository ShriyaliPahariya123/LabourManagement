...
export default function DisplayAllProduct() {
    ...
    const [stateCol, setStateCol] = React.useState({
        columns: [  { title: 'Subcategory ID', field: 'subcategoryId' ,editable:'never' },
            { title: 'Category Name', field: 'categoryName' ,editable:'never'},
            { title: 'Subategory Name', field: 'subcategoryName' },
          ]
  ...
  const readAllRecords=async()=>{
    var list=await getData('subcategory/displayallsubcategory1') 
    setState({data:list})
    }
 useEffect(()=>{
  readAllRecords()
